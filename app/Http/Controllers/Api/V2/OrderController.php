...
class OrderController extends Controller
{
    ...
    //    if ($cartItems->first()->address_id == 10 ){
    //     return "hiii";
    //    }else{
    //     return "hello";
    //    }
...
if($address){
        ...
        if ($address != null) {
            ...
            // $shippingAddress['state']       = "Kerala";//$address->state->name?$address->state->name:"Kerala";
        ...
        foreach ($seller_products as $seller_product) {
            ...
            foreach ($seller_product as $cartItem) {
                ...
                if($address){
                 $order_detail->shipping_cost = $Shipping_country_cost;
                }
                $shipping += $order_detail->shipping_cost;


                $cart_id = $cartItem['id'];
                $custom_value = CustomValue::where('cart_id', $cart_id)->first();
                if ($custom_value) {
                ...
                $order_detail->custom_id = $custom_value_id;
        ...
        if($request->payment_type == 'cash_on_delivery') {
            ...
            $apiSecret = env('RAZOR_SECRET');

            $data = [
                'amount' => 300*100, //a fixed amount for advance in cash on delivery orders
                'currency' => 'INR',
                'receipt' => 'advance_'.$combined_order->id,
            ];
        ...
        if($request->payment_type == 'razorpay') {
            ...
            $data = [
                ...
                'currency' => 'INR',
                'receipt' => 'rcpt_'.$combined_order->id,
            ];
        ...
        return response()->json([
            ...
            'payment_order' => json_decode($response->body()),
            'result' => true,
