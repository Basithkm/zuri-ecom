...
use App\Http\Resources\V2\ProductNewMiniCollection;
...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return new ProductMiniCollection($query->latest()->paginate(12));
    }

    public function homeNew()
    {
        return new ProductNewMiniCollection(Product::latest()->limit(4)->get());
    }
    ...
    {
        // print_r(Category::where('parent_id', $id)->get());die;
        return new SubCategoryCollection(Category::where('parent_id', $id)->get());
...

    {
        ...
        $products = Product::where('best_seller',1)->physical();
    ...
    public function festiveCollection(Request $request)
    {
        $category_ids = [];

        $products = Product::where('festive_collection',1)->physical();
    ...
    {
        $category_ids = [];
        $products = Product::where('custom',1)->physical();
    ...
    public function search(Request $request)
    {
        ...
        if ($request->attribute_values != null && $request->attribute_values != "") {
            $attributes = explode(',', $request->attribute_values);
        }
        ...
        if ($unit_price != null && $unit_price != "") {
            $products->where('unit_price', 'like', '%' . $unit_price . '%');
        }
        if ($name != null && $name != "") {
        ...
        if(!empty($attributes)) {
            $products->where(function ($query) use ($attributes) {
                foreach ($attributes as $attribute) {
                    $query->orWhere('variations', 'LIKE', "%$attribute%");
                }
            });
        }
